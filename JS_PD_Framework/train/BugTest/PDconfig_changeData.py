...
#
#     data1._getCols("Gross Profit")[~((X > 0) & (Y > 0))] = (
#                 XX[~((X > 0) & (Y > 0))] - YY[~((X > 0) & (Y > 0))].values).values
#     data1._getCols("Total Cost of Sales")[~((X > 0) & (Z > 0))] = (
#                 XX[~((X > 0) & (Z > 0))] - ZZ[~((X > 0) & (Z > 0))].values).values
#     data1._getCols("Total Sales / Turnover")[~((Y > 0) & (Z > 0))] = (
#                 YY[~((Y > 0) & (Z > 0))] - ZZ[~((Y > 0) & (Z > 0))].values).values
#
#     data1.additionClean("Total Non Current Assets", "Leasehold", "Freehold", "Land & Buildings", "Fixtures & Fittings",
#                         "Plant", \
#                         "Vehicles", "Plant & Machinery", "Other Tangible Assets", "Property, Plant & Equipment", \
#                         "Intangible Assets", "Other Non Current Assets")
#
#     data1.additionClean("Total Non Current Assets", \
#                         "Finished Goods", \
...
#     data1.additionClean("Total Assets", "Total Equity", "Total Non Current Liabilities (Incl Provisions)",
...
# Because we do not use macro variables in this model, we do not use the following function.
